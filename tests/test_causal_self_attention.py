...
from src.attention.causal_self_attention import CausalSelfAttention


def test_output_shape_matches_input():
    B, N, d_in, d_out = 2, 4, 8, 8
    attn = CausalSelfAttention(d_in=d_in, d_out=d_out, context_len=N)
...
def test_attention_is_causal():
    ...
    attn = CausalSelfAttention(d_in=d_in, d_out=d_out, context_len=N, dropout=0.0)
    x = torch.randn(B, N, d_in)
    ...
    assert torch.all(upper_triangle == 0), "position i attends to positions j > i"
...
def test_handles_sequence_shorter_than_context_len():
    context_len = 16
    B, N, d_in, d_out = 2, 4, 8, 8
    attn = CausalSelfAttention(d_in=d_in, d_out=d_out, context_len=context_len)
    x = torch.randn(B, N, d_in)
