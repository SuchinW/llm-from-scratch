...
class MultiQueryAttention(nn.Module):
    ...
    Reduces KV-cache memory and bandwidth drastically at inference time,
    at the cost of some quality vs vanilla MHA.
    """
    def __init__(self, d_in, d_out, n_heads, context_len, dropout = 0.0, qkv_bias = False):
        ...
        # Q has n_heads heads, K and V have a single head each
        self.q_proj = nn.Linear(d_in, d_out, bias = qkv_bias)
        self.k_proj = nn.Linear(d_in, self.d_head, bias = qkv_bias)
        self.v_proj = nn.Linear(d_in, self.d_head, bias = qkv_bias)
    ...
    def forward(self, x):
        ...
        # B, 1, N, d_head  (broadcast across heads)
        k = k.view(B, N, 1, self.d_head).transpose(1, 2)
        v = v.view(B, N, 1, self.d_head).transpose(1, 2)
